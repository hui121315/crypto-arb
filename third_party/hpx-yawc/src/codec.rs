...
mod tests {
    use super::Decoder;
    use crate::{Role, frame::OpCode};
    use bytes::BytesMut;
    use tokio_util::codec::Decoder as _;

    #[test]
    fn control_opcodes_survive_incremental_payload_reads() {
        for (wire_opcode, expected) in [
            (0x8, OpCode::Close),
            (0x9, OpCode::Ping),
            (0xA, OpCode::Pong),
        ] {
            let mut decoder = Decoder::new(Role::Client, 1 << 20);
            let mut buffer = BytesMut::from(&[0x80 | wire_opcode, 0x04][..]);
            ...
            assert_eq!(frame.into_payload().as_ref(), b"test");
        }
    }
}
