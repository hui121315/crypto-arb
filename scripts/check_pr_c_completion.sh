...
SKIP_UPSTREAM="$(printenv CROSSLINE_PR_C_SKIP_UPSTREAM 2>/dev/null || printf '0')"
if [ "$MODE" = "check" ] && [ "$SKIP_UPSTREAM" != "1" ]; then
  for gate in \
    check_pr_fd_completion.sh \
    check_pr_ep_completion.sh \
    check_pr_eo_completion.sh \
    check_pr_er_completion.sh \
    check_pr_fz_completion.sh; do
...
PR_TITLE = "PR-C ExecutionRun Finality"
EVIDENCE = {
    "execution-finality-core": (
        "scripts/check_pr_fd_completion.sh",
        "bash scripts/check_pr_fd_completion.sh",
    ),
    "htx-per-fill-ledger-policy": (
...
TEST_ANCHORS = (
    ...
    (
        "crates/api/src/trading_service/private_ws_mapper/tests/cases_htx.rs",
        "htx_match_notification_does_not_emit_fee_less_duplicate_fill",
    ),
    (
    ...
    (
        "crates/api/src/trading_service/private_ws_mapper/tests/cases_kucoin.rs",
        "kucoin_order_update_does_not_emit_fill_without_fee_evidence",
    ),
    (
        "crates/api/src/trading_service/private_ws_mapper/tests/cases_kucoin.rs",
        "kucoin_classic_match_projects_durable_fill_once",
    ...
    (
        ...
        "gate_usertrade_maps_to_private_fill_delta",
    ),
    (
        "crates/api/src/trading_service/private_ws_mapper/tests/cases_gate_account.rs",
    ...
    (
        ...
        "gate_fill_health_waits_for_identity_apply_and_durable_ack",
    ),
)
...
def require_roadmap(root: Path) -> None:
    ...
    for line in text[start:end].splitlines():
        if line.startswith("| `PR-C "):
            rows.append([cell.strip() for cell in line.strip().strip("|").split("|")])
    if len(rows) != 1:
        raise ValueError(f"expected exactly one PR-C roadmap row, found {len(rows)}")
    cells = rows[0]
    if len(cells) != 4:
        raise ValueError(f"PR-C roadmap row must have 4 cells, found {len(cells)}")
    if cells[0].strip("`") != PR_TITLE:
        raise ValueError(f"PR-C roadmap title drifted: {cells[0]!r}")
    if cells[1] != "✅ 完成":
        raise ValueError(f"PR-C roadmap status must be complete, got {cells[1]!r}")
    if "剩余：无。" not in cells[2]:
        raise ValueError("PR-C completion must declare no remaining local work")
    if "bash scripts/check_pr_c_completion.sh --self-test" not in cells[3]:
        raise ValueError("PR-C completion must record its destructive self-test")

    queue_start = text.find("### 🟡 6.5")
    if queue_start < 0:
        raise ValueError("missing 6.5 execution queue")
    queue_tail = text[queue_start:]
    if re.search(r"(?m)^\d+\.\s+\*\*PR-C\b", queue_tail):
        raise ValueError("completed PR-C must not remain in the 6.5 queue")
...
def require_evidence(root: Path) -> None:
    ...
    c_rows = [row for row in rows if row["pr_id"].strip() == "PR-C"]
    by_type: dict[str, dict[str, str]] = {}
    for row in c_rows:
        evidence_type = row["evidence_type"].strip()
        if evidence_type in by_type:
            raise ValueError(f"duplicate PR-C evidence type: {evidence_type}")
        by_type[evidence_type] = row

    if set(by_type) != set(EVIDENCE):
        raise ValueError(
            "PR-C evidence type drift: "
    ...
    for evidence_type, (artifact, command_anchor) in EVIDENCE.items():
        ...
        if row["artifact"].strip() != artifact:
            raise ValueError(
                f"PR-C {evidence_type} artifact must be {artifact}, "
                f"got {row['artifact'].strip()!r}"
            )
        if command_anchor not in row["command"]:
            raise ValueError(
                f"PR-C {evidence_type} command must contain {command_anchor!r}"
            )
        if not (root / artifact).is_file():
            raise ValueError(f"PR-C {evidence_type} artifact is missing: {artifact}")
...
def require_runnable_tests(root: Path) -> None:
    for relative_path, function_name in TEST_ANCHORS:
        ...
        if not path.is_file():
            raise ValueError(f"missing PR-C test anchor file: {relative_path}")
        text = path.read_text(encoding="utf-8")
        pattern = re.compile(
            r"(?m)^\s*#\[(?:tokio::)?test(?:\([^\]]*\))?\]\s*\n"
            r"(?P<attrs>(?:\s*#\[[^\n]+\]\s*\n)*)"
        ...
        if match is None:
            raise ValueError(
                f"missing runnable PR-C test anchor {relative_path}::{function_name}"
            )
        if "ignore" in match.group("attrs").lower():
            raise ValueError(
                f"PR-C test anchor must not be ignored: {relative_path}::{function_name}"
            )
...
def require_coverage(root: Path) -> None:
    ...
    matches = [
        row
        for row in rows
        if row["file"].strip() == "scripts/check_pr_c_completion.sh"
    ]
    if len(matches) != 1 or matches[0]["coverage_status"].strip() != "exact":
        raise ValueError("PR-C completion gate requires exact coverage ledger ownership")


def require_upstream_gates(root: Path) -> None:
    for relative_path in UPSTREAM_GATES:
        if not (root / relative_path).is_file():
            raise ValueError(f"missing PR-C upstream completion gate: {relative_path}")


def check(root: Path) -> None:
    if os.environ.get("CROSSLINE_PR_C_SKIP_UPSTREAM") != "1":
...
def stage_fixture_docs(root: Path) -> None:
    ...
    (docs / "PRODUCT_FULL_AUDIT_REFINEMENT.md").write_text(
        ...
        "| PR | 状态 | 范围 | 验收标准 |\n"
        "|---|---|---|---|\n"
        "| `PR-C ExecutionRun Finality` | ✅ 完成 | durable finality handoff; 剩余：无。 | "
        "验证：`bash scripts/check_pr_c_completion.sh --self-test` |\n\n"
        "### 🟡 6.4 下一批继续审计重点\n\n"
        "### 🟡 6.5 下一步执行队列\n",
        encoding="utf-8",
    )
    ...
    evidence.write_text(
        "pr_id\tevidence_type\tartifact\tcommand\tnotes\n"
        + "".join(
            f"PR-C\t{evidence_type}\t{artifact}\t{command}\tfixture\n"
    ...
    (docs / "PRODUCT_AUDIT_COVERAGE.tsv").write_text(
        ...
        "scripts/check_pr_c_completion.sh\texact\tverification_script\tP0\tPR-GI/PR-AW\t"
...
def run_fixture(root: Path, expect_pass: bool) -> None:
    ...
    environment["CROSSLINE_PR_C_SKIP_UPSTREAM"] = "1"
    ...
    if expect_pass != (result.returncode == 0):
        ...
        raise ValueError(f"PR-C destructive self-test expectation failed: {detail}")


def self_test(root: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="crossline-pr-c-completion-") as temp:
        ...
        evidence.write_text(
            "\n".join(
                ...
                if not line.startswith("PR-C\thtx-per-fill-ledger-policy\t")
            )
            + "\n",
            encoding="utf-8",
        )
        ...
        doc.write_text(
            doc_baseline.replace(
                "| `PR-C ExecutionRun Finality` | ✅ 完成 |",
                "| `PR-C ExecutionRun Finality` | 🟡 部分完成 |",
                1,
            ),
            encoding="utf-8",
        )
        ...
        anchor = fixture / "crates/api/src/trading_service/private_ws_mapper/tests/cases_htx.rs"
        anchor_baseline = anchor.read_text(encoding="utf-8")
        anchor.write_text(
            anchor_baseline.replace(
                "#[test]\nfn htx_settled_order_maps_stable_fill_identity_and_reported_fee",
                "#[test]\n#[ignore]\nfn htx_settled_order_maps_stable_fill_identity_and_reported_fee",
                1,
            ),
            encoding="utf-8",
        ...
        coverage.write_text(
            coverage_baseline.replace(
                "scripts/check_pr_c_completion.sh\texact\t",
                "scripts/check_pr_c_completion.sh\tmissing\t",
                1,
            ),
            encoding="utf-8",
        )
    ...
    print("OK PR-C completion destructive self-test")
