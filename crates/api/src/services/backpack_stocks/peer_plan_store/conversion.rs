...
pub(in crate::services::backpack_stocks) fn client_id(p: &StockPeerPlan, index: usize) -> String {
    ...
    format!(
        "fc{}",
        &common::signing::hmac_sha256_hex(b"stock-peer-fx-v1", text.as_bytes())[..16]
    )
}
impl Store {
    pub(in crate::services::backpack_stocks) fn conversion_receipt(
        &self,
        id: &str,
        index: usize,
        ...
        self.update(id, |p| {
            let c = p.conversions.get_mut(index).ok_or("原换汇不存在")?;
            let old = c.order.as_mut().ok_or("换汇尚未提交")?;
...
pub(super) fn validate_history(p: &StockPeerPlan) -> Result<(), String> {
    if p.conversions.len() > MAX_STOCK_PEER_CONVERSIONS
        || (!p.conversions.is_empty() && p.phase != StockPeerPlanPhase::SubmissionUnknown)
    {
        return Err("换汇历史数量或父计划状态无效".into());
    }
    for (index, c) in p.conversions.iter().enumerate() {
        let at = c.draft.prepared_at_ms;
        if c.request.revision >= p.revision || at < p.terms.created_at_ms || at > p.updated_at_ms {
            return Err("换汇历史版本或时间无效".into());
        }
        let prefix = historical_prefix(p, c.request.revision);
        let rebuilt = StockPeerConversion::compile(
            &prefix,
            c.request.clone(),
            c.market.clone(),
            c.quote.clone(),
        ...
        {
            return Err("换汇查询历史无效".into());
        }
    }
    Ok(())
}
pub(super) fn transition(a: &StockPeerPlan, b: &StockPeerPlan) -> bool {
    if b.conversions.len() < a.conversions.len() || b.conversions.len() > a.conversions.len() + 1 {
        return false;
    }
    for (old, new) in a.conversions.iter().zip(&b.conversions) {
    ...
    if let Some(c) = b.conversions.get(a.conversions.len()) {
