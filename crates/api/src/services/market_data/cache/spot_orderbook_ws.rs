...
impl MarketDataCache {
    pub(crate) async fn refresh_spot_orderbook_from_ws(
        &self,
        ...
        if let Some(read) = self.ready_ws_spot_orderbook(&key, now_ms) {
            return read;
        }

        let guard_symbol = format!("{venue}:{symbol}");
        let guard_key = MarketKey::spot("ws-spot-orderbook", &guard_symbol);
        let _guard = self.rest_baseline.orderbook_guard(&guard_key).await;
        let refresh_ms = common::time::now_ms();
        if let Some(read) = self.ready_ws_spot_orderbook(&key, refresh_ms) {
            return read;
        }

        self.record_cache_miss_for_feed(FEED_SPOT_ORDERBOOK);
        let book = match fetch_first_ws_spot_orderbook(producer, venue, symbol, depth).await {
            Ok(book) => book,
            Err(failure) => {
                let failed_at_ms = common::time::now_ms();
                self.record_spot_orderbook_error(
                    &key,
                    failed_at_ms,
                    failure.quality,
                    failure.backoff_ms,
        ...
        let refresh_ms = common::time::now_ms();
        self.spot_orderbook_fetch_backoff.remove(&key);
    ...
    fn ready_ws_spot_orderbook(
        &self,
        key: &MarketKey,
        now_ms: i64,
    ) -> Option<MarketRead<OrderBookInfo>> {
        self.reusable_ws_spot_orderbook(key, now_ms)
            .or_else(|| self.blocked_ws_spot_orderbook(key, now_ms))
    }

    fn reusable_ws_spot_orderbook(
        &self,
        key: &MarketKey,
        now_ms: i64,
    ) -> Option<MarketRead<OrderBookInfo>> {
        let read = read_entry(&self.spot_orderbooks, key, now_ms, ORDERBOOK_WS_REUSE_MS)?;
    ...
    fn blocked_ws_spot_orderbook(
        &self,
        key: &MarketKey,
        now_ms: i64,
    ) -> Option<MarketRead<OrderBookInfo>> {
        let backoff = fetch_backoff_wait(&self.spot_orderbook_fetch_backoff, key, now_ms)?;
        Some(self.read_spot_orderbook_after_blocked_fetch(
            key,
            now_ms,
...
async fn fetch_first_ws_spot_orderbook(
    producer: &dyn MarketDataSource,
    venue: &str,
    symbol: &str,
    depth: u32,
    ...
    for attempt in 0..WS_ORDERBOOK_ATTEMPTS {
        match producer.fetch_ws_spot_orderbook(venue, symbol, depth).await {
            ...
            Ok(exchange::PublicWsSnapshot::Unsupported) => {
                return Err(WsOrderbookFailure {
                    ...
                    error: format!("{venue} {symbol} does not expose spot WS depth"),
                });
            }
            Err(error) => {
                tracing::debug!(%venue, %symbol, error = %error, "spot orderbook WS snapshot unavailable");
                return Err(WsOrderbookFailure {
    ...
    Err(WsOrderbookFailure {
        ...
        error: format!(
            "{venue} {symbol} spot WS depth produced no first frame within {}ms",
