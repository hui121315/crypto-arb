...
- 配对平仓只在点击的持仓行展开确认；另一条腿不会重复展开，实际提交仍锁住整个配对，避免双腿重复提交。
...
- 从提醒进入时按原机会 ID 定位，并清掉无关的旧筛选；加载中或原机会不在当前列表时，不会自动选中另一条。可主动取消定位或选择其他行，重新预检后再构建。
...
后台用同一机会的新快照完成预检时，明确回传原请求快照，前端核对后接纳新票据，不额外重复预检。
同一市场只更新快照编号时，保留当前输入，不把小额本金重置为默认金额；重新选择不同市场或报价仍隔离旧草稿。
名义金额按本金与杠杆保留小数，不取整到美元或擅自增至 1 美元；交易所数量步长及最小下单量仍由后台预检核对。
相同参数预检未返回时，重复刷新共用在途请求；快照连续变化最多自动重试两次，随后明确提示并等待手动刷新，
不循环构建票据。更新后的票据仍须重新校验确认，保留输入不等于沿用旧提交资格。
...
- 闭环跟随具体运行回执，暂停或关闭自动化不抹掉已成交、已退出的记录；原票据只读，Webhook 证据按同一票据关联。
  ...
  退出详情显示记录中的止盈、止损或强平距离保护原因；触发原因不代表订单已成交或策略已盈利。
