...
pub(super) const API_SLOT_LABEL: &str = "交易接口";
        ...
        if total > 0 {
            ...
            if configured == 0 {
                if snapshot
                    .rows
                    .iter()
                    .any(|row| is_api_operation_row(row) && row.configured.is_none())
                {
                    return "配置待确认".into();
                }
...
pub(super) fn is_authenticated_api_transport_row(row: &VenueOperationHealth) -> bool {
