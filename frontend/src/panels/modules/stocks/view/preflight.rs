...
pub(super) fn panel(asset: String, data: StockData) -> impl IntoView {
    ...
    view! {<section class="stock-section stock-preflight" aria-label="股票库存与成本预检">
        <header><h3>"库存与成本"</h3><span>{move ||if draft.pending.get(){"处理中"}else if current.get(){"本次预检快照"}else if report.with(Option::is_some){"历史快照 · 构建时自动复核"}else{"尚未预检"}}</span></header>
        <form class="stock-preflight-form" on:submit=move |ev|{ev.prevent_default();draft.read.run(read_asset.clone());}>
            <label><span>"Solana 钱包地址"</span><input type="text" autocomplete="off" placeholder="公开地址" aria-label="股票套利 Solana 钱包地址"
                value=move ||draft.wallet.get() prop:value=move ||draft.wallet.get() on:input=move |ev|draft.wallet.set(event_target_value(&ev)) disabled=move ||draft.pending.get()/></label>
            <button type="submit" class="row-action" disabled=move ||draft.pending.get() || data.pending.get()>{move ||if draft.pending.get(){"正在预检…"}else{"检查库存与成本"}}</button>
        </form>
        <p class="stock-rfq-note">"只读预检 · 未签名、未下单"</p>
        ...
        {move ||report.get().map(|p|{
            ...
            view!{<div class="stock-quote-meta"><span>{format!("账户现货 taker 费率 {fee}")}</span></div>
        ...
        <div class="stock-directions">{[StockChainDirection::Buy, StockChainDirection::Sell].into_iter().map(|direction| {
            ...
            let cost_current=Memo::new(move |_|data.market.with(|m|m.value().is_some_and(|s|
            ...
            let build_problem=Memo::new(move |_| {
                ...
                if data.quote_pending.get() { return Some("正在更新询价，请等待完整结果"); }
            ...
            view!{<article class="stock-direction">
                ...
                {move ||cost.get().map(|c| {
                    ...
                    view!{<div class="stock-chain-cost" data-current=move ||cost_current.get().to_string()>
                        ...
                        <details><summary>"费用明细与付款方"</summary><ul>
                            <li>{format!("Provider 预算估算 {} SOL · 不代替周转余额",sol(c.wallet_budget_lamports))}</li>
                        ...
                        <p class="stock-rfq-note">{move ||if has_native_budget {
                            ...
                            else if zero_native_budget {"模拟净扣为零不代表无需备款；周转余额单独核验。"}
                ...
                {move ||row.get().map(|r|view!{
                    <p class="stock-rfq-note" role="status">{move ||if current.get(){"本次预检预算 · 不是实际收益"}else{"历史预检 · 不代表当前余额与可执行差额"}}</p>
                    <dl class="stock-direction-values"><div><dt>"另计交易所费用 / USDC"</dt><dd>{r.cex_fee_usdc.unwrap_or_else(||"未知".into())}</dd></div>
                        ...
                        <div><dt>"已知费用后差额 / USDC"</dt><dd>{r.after_known_costs_usdc.unwrap_or_else(||"—".into())}</dd></div></dl>
                ...
                {move ||funding.get().filter(|f|!f.needs.is_empty()).map(|f|super::funding::needs(f,data,funding_asset.clone(),report.get().map_or(0,|p|p.checked_at_ms)))}
            </article>}
        }).collect_view()}</div>
    </section>}
}
