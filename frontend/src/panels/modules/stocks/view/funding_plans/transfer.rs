        ...
        {t.map(|t|{
            ...
            view!{<div class="stock-order-receipt stock-funding-transfer">
                ...
                {waiting.then(||view!{<dl class="stock-plan-evidence">
                    ...
                    <div><dt>"RPC 接收"</dt><dd>{if t.acknowledged{"已接收 · 非到账证明"}else if chain_checked{"原回复未确认 · 已核对链上终态"}else{"回复未确认"}}</dd></div>
                    {t.receipt.map(|r|view!{
                        <div><dt>"链上终态"</dt><dd>{if r.succeeded{"已最终确认"}else{"已失败"}}</dd></div>
                    ...
                    <div><dt>"Backpack 入账状态"</dt><dd>{t.deposit.as_ref().map(|d|d.status.clone()).unwrap_or("待核验".into())}</dd></div>
                ...
                {(waiting && holds).then(||view!{<button type="button" class="row-action" disabled=move||data.preflight.pending.get() || data.clock.get()<next
                    on:click=move |_|data.preflight.funding_recheck.run(read.clone())>"核对原转账与 Backpack 入账"</button>})}
            </div>}
        })}
    }
}
