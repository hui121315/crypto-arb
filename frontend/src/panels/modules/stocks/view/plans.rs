...
pub(super) fn panel(data: StockData) -> impl IntoView {
    ...
    view! {<section class="stock-section stock-plans" aria-label="股票执行计划">
        ...
        {picker}
        <For each=move ||plans.with(|plans|plans.iter().filter(|p|Some(&p.plan_id)==selected.get().as_ref()).cloned().collect::<Vec<_>>()) key=|p|(p.plan_id.clone(),p.revision) children=move |plan| {
            let review_href=crate::panels::routing::settlement_review_href(shared_types::review::settlements::SettlementSource::Stocks,&plan.plan_id);
            ...
            let recheck_label=if plan.two_leg_started_at_ms.is_some(){"核对两腿处理结果"}else if plan.chain_submission.is_some(){"核对原链上交易"}else if plan.rfq_acceptance.is_some(){"核对原 询价"}else{"核对原订单"};
            ...
            let cost_source_plan=plan.clone();
            let cost_problem=Memo::new(move |_| {
                if settled || cost_source_plan.terms.conversion_costs.is_empty() {return None;}
                data.market.with(|m| m.value().map(|s| s.exchange_conversion_problem.clone()
                    .or_else(||cost_source_plan.check_conversion_sources(&s.exchange_conversions).err()))
                    .unwrap_or(Some("等待前期费用记录".into())))
            });
            ...
            let restock_request=StockPreflightRequest{asset:plan.request.asset.clone(),wallet_address:Some(plan.request.wallet_address.clone()),source_plan:Some(plan.inventory_source())};
            let restock_asset=plan.request.asset.clone();
            let topups=native_topups(&plan, data);
            let recovery=super::recovery::panel(data,&plan);
            let fee=plan.terms.cex_fee_budget.clone();
            let conversion_fee=(!plan.terms.conversion_costs.is_empty()).then(||plan.terms.conversion_fee_usdc()
                .map(|n|n.normalize().to_string()).unwrap_or_else(|_|"待核实".into()));
            ...
            let dom_id=format!("stock-plan-{}",plan.plan_id);
            view!{<article class="stock-plan-record" id=dom_id>
                <header><div><strong>{format!("{} · {}",plan.request.asset,plan.request.direction.label())}</strong>
                    ...
                    {settled.then(||view!{<button type="button" class="row-action" title="重读原账户和钱包，按本笔规模检查库存与充提；不下单或转账"
                        disabled=move ||data.pending.get() ||data.preflight.pending.get() ||data.market.with(|m|m.value().and_then(|s|s.security.as_ref()).is_none_or(|s|s.asset!=restock_asset))
                        on:click=move |_|{data.section.set(1);data.preflight.wallet.set(restock_request.wallet_address.clone().unwrap_or_default());data.preflight.restock.run(restock_request.clone());
                    ...
                    {(can_settle && !settled).then(||view!{<button type="button" class="row-action" disabled=move ||data.preflight.pending.get() || cost_problem.get().is_some() on:click=move |_|data.preflight.settle.run(settle_request.clone())>"结束并释放预留"</button>})}
                    {super::history::close(data.preflight.selected_plan)}
                    </div>
                </header>
                <p class="stock-rfq-note">{move ||if phase.get()==StockPlanPhase::Reserved {format!("备款剩余 {} 秒 · {}",reserved_until.saturating_sub(data.clock.get()).max(0)/1000,if data.clock.get()>=valid_until{"报价已过期，执行前需重新核对"}else{"报价仍有效，尚非执行许可"})}else if settled{"预留已释放 · 收支与备款已归档".into()}else{"保留原计划与资金记录，不自动重发".into()}}</p>
                ...
                {execution}
                {accounting}
                {move ||cost_problem.get().map(|p|view!{<p role="alert" class="stock-problem">{format!("前期费用待核对：{p}")}</p>})}
                {recovery}
                {topups}
                {receipt}
                {rfq}
                ...
                <details><summary>"计划凭据"</summary><dl class="stock-plan-evidence">
                    ...
                    {conversion_fee.map(|fee|view!{<div><dt>"已计入前期换币手续费 / USDC"</dt><dd>{fee}</dd></div>})}
                    {plan.terms.conversion_costs.into_iter().map(|p|view!{<div><dt>"原兑换费用记录"</dt><dd>{p.plan_id}</dd></div>}).collect_view()}
                    {fee.map(|f|view!{<div><dt>"交易所费用预算"</dt><dd>{match f.basis {StockCexFeeBasis::OrderBookQuote{..}=>format!("{} USDC · 实扣以处理结果为准",f.additional_fee),StockCexFeeBasis::RfqIncluded{..}=>"询价 报价已含费，不再叠加现货费".into()}}</dd></div>
...
fn accounting(plan: &StockExecutionPlan) -> impl IntoView {
    ...
    view! {<div class="stock-order-receipt" aria-label="双腿实际收支">
        ...
        <p class="stock-rfq-note">"本次资金变化与前期费用分开记录；未归集费用、换币价差及库存再平衡不在该差额内。"</p>
...
fn chain_receipt(plan: &StockExecutionPlan) -> impl IntoView {
    ...
    plan.chain_submission.clone().map(move |row| {
        let status=match &row.receipt {
            ...
            None if row.provider_acknowledged=>"报价服务已回复 · 链上结果待核对",
            None=>"链上提交结果未知 · 不重复发送",
        };
        let id=row.transaction_id.clone().unwrap_or_else(||"待原交易处理结果确认".into());
        view!{<div class="stock-order-receipt" aria-label="链上交易处理结果">
            <strong>{status}</strong><span class="stock-rfq-note">{if settled {"交易收支已归档 · 预留已释放"}else{"两边资金尚未核清 · 保留占用"}}</span>
            ...
            {row.receipt.map(move |r|view!{
                ...
                <details><summary>"链上处理结果凭据"</summary><dl class="stock-plan-evidence">
...
fn order_receipt(plan: &StockExecutionPlan) -> impl IntoView {
    ...
    plan.cex_order.clone().map(move |order| {
        ...
        let status=if order.evidence_conflict {"处理结果冲突 · 停止自动处理"} else {match order.phase {
        ...
        view!{<div class="stock-order-receipt" aria-label="交易所订单处理结果">
            ...
            <dl class="stock-plan-evidence">
                ...
                <div><dt>"处理结果核对"</dt><dd>{review}</dd></div>
            </dl>
...
pub(super) fn rfq_receipt(plan: &StockExecutionPlan) -> impl IntoView {
    ...
    plan.rfq_acceptance.clone().map(move |r| {
        ...
        let review=if accepted.as_ref().is_some_and(|a|a.evidence_conflict){"处理结果冲突 · 保留原记录".into()}
            else if complete{"成交数量已核对 · 费用与净到账待核实".into()}
            else if r.settlement.paused{"自动核对已暂停，可手动核对原 询价".into()}
        ...
        view!{<div class="stock-order-receipt" aria-label="询价 接受与结算处理结果">
            ...
            <details><summary>{format!("原始 询价 成交 · {} 笔",r.fills.len())}</summary><dl class="stock-plan-evidence">
...
fn instruction_label(instruction: Option<&StockCexInstruction>) -> String {
    match instruction {
        ...
        Some(StockCexInstruction::AcceptRfq {
            ...
            "{symbol} · {} {quantity} 股 · {taker_price} USDC · 询价 {rfq_id} · 报价 {quote_id}",
            if *side == StockRfqSide::Ask {
                "卖"
            } else {
                "买"
